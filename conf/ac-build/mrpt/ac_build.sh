...
build() {
	...
	cmake -C ../${target}.initial.cmake -DCMAKE_TOOLCHAIN_FILE=../$target.toolchain.cmake \
		...
		-DDISABLE_OPENCV:BOOL=TRUE -DDISABLE_OPENGL:BOOL=TRUE \
		-DBUILD_EXAMPLES:BOOL=FALSE \
		-DBUILD_KINECT:BOOL=FALSE -DBUILD_HWDRIVERS:BOOL=FALSE \
		-DWITH_JASPER:BOOL=FALSE -DWITH_TIFF:BOOL=FALSE -DWITH_FFMPEG:BOOL=FALSE \
		-DBUILD_MONOSLAM:BOOL=TRUE \
		-DPROG_POD2MAN= \
